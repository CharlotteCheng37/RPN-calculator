    ...
    public static void evaluateCsv(CSVParser parser, CSVPrinter printer) throws IOException {
        ...
        for (CSVRecord record : parser) {
            ...
            for (String cell : record) {
                String cellName = colToLetters(colIndex) + rowIndex;

                if (!cell.startsWith("=")) {
                    try {
                        double val = Double.parseDouble(cell);
                        vars.set(cellName, val);
                    } catch (NumberFormatException e) {
                        // Not a formula. Not a number.
                    }
                    ...
                    try {
                        Expression expr = RpnParser.parse(exprString, defs);
                        double val = expr.eval(vars);
                        vars.set(cellName, val);
                        printer.print(Double.toString(val));
                    } catch (Exception e) {
                        printer.print("#N/A");
                    }
                }
